...
module ConnectorContracts
  class Index < Dry::Validation::Contract
    ...
    rule(:type) do
      unless !key? || Outhad::Integrations::Protocol::ConnectorType.include?(value.downcase)
  ...
  class Create < Dry::Validation::Contract
    ...
    rule(connector: :connector_type) do
      unless Outhad::Integrations::Protocol::ConnectorType.include?(value.downcase)
        key.failure("invalid connector type")
      end
    end

    rule(connector: %i[connector_type connector_name]) do
      if value.first.downcase == Outhad::Integrations::Protocol::ConnectorType["source"]
        unless Outhad::Integrations::ENABLED_SOURCES.include?(value.second)
          key.failure("invalid connector source name")
        end
      elsif value.first.downcase == Outhad::Integrations::Protocol::ConnectorType["destination"]
        unless Outhad::Integrations::ENABLED_DESTINATIONS.include?(value.second)
  ...
  class Update < Dry::Validation::Contract
    ...
    rule(connector: %i[connector_type connector_name]) do
      if key? && value.first.downcase == Outhad::Integrations::Protocol::ConnectorType["source"]
        unless Outhad::Integrations::ENABLED_SOURCES.include?(value.second)
          key.failure("invalid connector source name")
        end
      elsif key? && value.first.downcase == Outhad::Integrations::Protocol::ConnectorType["destination"]
        unless Outhad::Integrations::ENABLED_DESTINATIONS.include?(value.second)
          key.failure("invalid connector destination name")
        end
      end
