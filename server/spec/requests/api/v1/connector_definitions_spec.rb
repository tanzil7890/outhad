...
RSpec.describe "Api::V1::ConnectorDefinitions", type: :request do
  ...
  let(:service) { Outhad::Integrations::Service }
  let(:connection_status) { Outhad::Integrations::Protocol::ConnectionStatus }
  ...
  describe "POST /api/v1/connector_definitions/check_connection" do
    let(:mock_connector_class) { class_double("Outhad::Integrations::Source::Snowflake::Client") }
    let(:mock_connector_instance) { instance_double("Outhad::Integrations::Source::Snowflake::Client") }

    before do
      allow(Outhad::Integrations::Service).to receive(:connector_class)
    ...
    it "returns success status for a valid connection" do
      allow(mock_connector_instance).to receive(:check_connection)
        .and_return(connection_status.new(status: "succeeded").to_outhad_message)
    ...
    it "returns success status for a valid connection with env " do
      ...
      allow(mock_connector_instance).to receive(:check_connection).with({ test: "test_credentails" }).and_return(connection_status.new(status: "succeeded").to_outhad_message)
    ...
    it "returns failure status for a valid connection" do
      allow(mock_connector_instance).to receive(:check_connection)
        .and_return(connection_status.new(status: "failed").to_outhad_message)
