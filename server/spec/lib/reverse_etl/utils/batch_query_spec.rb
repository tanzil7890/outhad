    ...
    RSpec.describe BatchQuery do
      describe ".execute_in_batches" do
        ...
        context "when both cursor_field is empty" do
          ...
          let(:record) do
            Outhad::Integrations::Protocol::RecordMessage.new(data: { "id" => 1, "email" => "test1@mail.com",
                                                                          "first_name" => "John",
                                                                          "Last Name" => "Doe" },
                                                                  emitted_at: DateTime.now.to_i).to_outhad_message
          end
          let(:record1) do
            Outhad::Integrations::Protocol::RecordMessage.new(data: { "id" => 1, "email" => "test1@mail.com",
                                                                          "first_name" => "John",
        ...
        context "when both cursor_field is present" do
          ...
          it "executes batches and call CursorQueryBuilder" do
            ...
            BatchQuery.execute_in_batches(params) do |result, current_offset, last_cursor_field_value|
              expect(result.first).to be_an_instance_of(Outhad::Integrations::Protocol::OuthadMessage)
      ...
      describe ".extract_last_cursor_field_value" do
        let(:sync_config) { instance_double(Outhad::Integrations::Protocol::SyncConfig, cursor_field: "timestamp") }
        ...
        context "when sync_config has no cursor field" do
          let(:sync_config) { instance_double(Outhad::Integrations::Protocol::SyncConfig, cursor_field: nil) }
          let(:result) do
            [instance_double(Outhad::Integrations::Protocol::RecordMessage, data: { "timestamp" => "2022-01-01" })]
          end
      ...
      describe ".build_cursor_sync_config" do
        ...
        it "builds a new SyncConfig with modified query and other attributes" do
          ...
          expect(modified_sync_config).to be_a(Outhad::Integrations::Protocol::SyncConfig)
