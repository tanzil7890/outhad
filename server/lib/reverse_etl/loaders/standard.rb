  ...
  module Loaders
    class Standard < Base
      ...
      def handle_response(report, sync_run)
        is_outhad_tracking_message = report.is_a?(Outhad::Integrations::Protocol::OuthadMessage) &&
                                         report.type == "tracking" &&
                                         report.tracking.is_a?(Outhad::Integrations::Protocol::TrackingMessage)
        raise_non_retryable_error(report, sync_run) unless is_outhad_tracking_message
        report
      end
