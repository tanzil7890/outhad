...
module Outhad
  module Integrations::Protocol
    RSpec.describe ConnectionStatus do
      ...
      describe "#to_outhad_message" do
        ...
        it "converts to a OuthadMessage" do
          outhad_message = connection_status.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("connection_status")
          expect(outhad_message.connection_status).to eq(connection_status)
        end
      end
    end
    ...
    RSpec.describe ConnectorSpecification do
      ...
      describe "#to_outhad_message" do
        it "converts to a OuthadMessage" do
          ...
          outhad_message = connector_spec.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("connector_spec")
          expect(outhad_message.connector_spec).to eq(connector_spec)
        end
      end
    end
    RSpec.describe LogMessage do
      ...
      describe "#to_outhad_message" do
        ...
        it "converts to a OuthadMessage" do
          outhad_message = log_message.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("log")
          expect(outhad_message.log).to eq(log_message)
        end
      end
    end
    ...
    RSpec.describe RecordMessage do
      ...
      describe "#to_outhad_message" do
        it "converts to a OuthadMessage" do
          ...
          outhad_message = record.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("record")
          expect(outhad_message.record).to eq(record)
        end
      end
    end
    ...
    RSpec.describe Catalog do
      ...
      describe "#to_outhad_message" do
        it "converts to a OuthadMessage" do
          ...
          outhad_message = catalog.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("catalog")
          expect(outhad_message.catalog).to eq(catalog)
        end
      end
    end
    ...
    RSpec.describe Outhad::Integrations::Protocol::ControlMessage do
      json_data = {
        "type": "rate_limit",
        "emitted_at": 1_638_449_455_000,
        "meta": { "key": "value" },
      ...
      describe "#to_outhad_message" do
        it "converts to a OuthadMessage" do
          ...
          outhad_message = control.to_outhad_message

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("control")
          expect(outhad_message.control).to eq(control)
        end
      end
    end

    RSpec.describe Outhad::Integrations::Protocol::OuthadMessage do
      describe ".from_json" do
        let(:json_data) do
          {
            "type": "log",
        ...
        it "creates an instance from JSON" do
          outhad_message = described_class.from_json(json_data)

          expect(outhad_message).to be_a(described_class)
          expect(outhad_message.type).to eq("log")
        end
      end
    end
  ...
  RSpec.describe Outhad::Integrations::Protocol::TrackingMessage do
    describe "#to_outhad_message" do
      ...
      let(:tracking_message) do
        Outhad::Integrations::Protocol::TrackingMessage.new(success: 3, failed: 1, logs: [log_message_data])
      end

      it "converts to a OuthadMessage" do
        outhad_message = tracking_message.to_outhad_message
        expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
        expect(outhad_message.type).to eq("tracking")
        expect(outhad_message.tracking).to eq(tracking_message)
        expect(outhad_message.tracking.logs.first).to be_a(Outhad::Integrations::Protocol::LogMessage)
        expect(outhad_message.tracking.logs.first.level).to eq("info")
        expect(outhad_message.tracking.logs.first.message)
