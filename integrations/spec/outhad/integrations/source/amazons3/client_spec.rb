...
RSpec.describe Outhad::Integrations::Source::AmazonS3::Client do
  let(:client) { Outhad::Integrations::Source::AmazonS3::Client.new }
  let(:auth_data) do
  ...
  describe "#check_connection" do
    ...
    context "when the connection is successful for 'user' auth_type" do
      it "returns a succeeded connection status" do
        allow_any_instance_of(Outhad::Integrations::Source::AmazonS3::Client).to receive(:get_auth_data).and_return(auth_data)
        allow_any_instance_of(Outhad::Integrations::Source::AmazonS3::Client).to receive(:get_results).and_return([{ Id: "1" }, { Id: "2" }])
    ...
    context "when the connection fails" do
      it "returns a failed connection status with an error message" do
        allow_any_instance_of(Outhad::Integrations::Source::AmazonS3::Client).to receive(:get_auth_data).and_raise(StandardError, "Connection failed")
        message = client.check_connection(sync_config[:source][:connection_specification])
        result = message.connection_status
  ...
  describe "#read" do
    ...
    context "when reading unstructured data" do
      ...
      it "lists files successfully" do
        s_config = Outhad::Integrations::Protocol::SyncConfig.from_json(unstructured_sync_config.to_json)
  ...
  describe "#discover" do
    ...
    context "when discovering unstructured data" do
      it "returns unstructured stream for unstructured data" do
        ...
        expect(message.catalog.streams.first).to be_a(Outhad::Integrations::Protocol::Stream)
      end
    end
  end
