...
RSpec.describe Outhad::Integrations::Source::AwsAthena::Client do
  let(:client) { Outhad::Integrations::Source::AwsAthena::Client.new }
  let(:sync_config) do
    {
      "source": {
        "name": "AWS Athena",
  ...
  describe "#check_connection" do
    context "when the connection is successful" do
      it "returns a succeeded connection status" do
        allow_any_instance_of(Outhad::Integrations::Source::AwsAthena::Client).to receive(:create_connection).and_return(athena_client)
    ...
    context "when the connection fails" do
      it "returns a failed connection status with an error message" do
        allow_any_instance_of(Outhad::Integrations::Source::AwsAthena::Client).to receive(:create_connection).and_raise(StandardError, "Connection failed")
        message = client.check_connection(sync_config[:source][:connection_specification])
        result = message.connection_status
