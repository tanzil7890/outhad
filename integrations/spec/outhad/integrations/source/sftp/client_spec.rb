...
RSpec.describe Outhad::Integrations::Source::Sftp::Client do
  include WebMock::API
  ...
  let(:sync_config) do
    { source: {
      ...
      model: {
        name: "ExampleModel",
        query: "SELECT col1, col2, col3 FROM /outhad/test.csv",
        query_type: "raw_sql",
        primary_key: "id"
      },
      stream: {
  ...
  describe "#discover" do
    it "returns a catalog" do
      ...
      expect(first_stream.name).to eq("/outhad/test.csv")
  ...
  describe "#read" do
    it "reads records successfully" do
      ...
      expect(message.first).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
    end
