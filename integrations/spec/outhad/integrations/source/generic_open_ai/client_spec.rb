...
RSpec.describe Outhad::Integrations::Source::GenericOpenAI::Client do
  include WebMock::API
  ...
  let(:sync_config) { Outhad::Integrations::Protocol::SyncConfig.from_json(sync_config_json.to_json) }
  let(:sync_config_stream) do
    sync_config_json[:source][:connection_specification][:is_stream] = true
    Outhad::Integrations::Protocol::SyncConfig.from_json(sync_config_json.to_json)
  end
  before do
  ...
  describe "#read with is_stream = true" do
    context "when the read is successful" do
      before do
        ...
        allow(Outhad::Integrations::Core::StreamingHttpClient).to receive(:request)
          .with(endpoint,
                "POST",
      ...
      it "streams data and processes chunks" do
        ...
        expect(results.first.first.record).to be_a(Outhad::Integrations::Protocol::RecordMessage)
        expect(results.first.first.record.data.dig("choices", 0, "delta", "content")).to eq("How I ")

        expect(results[1]).to be_an(Array)
        expect(results[1].first.record).to be_a(Outhad::Integrations::Protocol::RecordMessage)
        expect(results[1].first.record.data.dig("choices", 0, "delta", "content")).to eq("can help ")

        expect(results[2]).to be_an(Array)
        expect(results[2].first.record).to be_a(Outhad::Integrations::Protocol::RecordMessage)
    ...
    context "when the read is successful but failed message for open ai" do
      ...
      it "streams data failed" do
        ...
        expect(result).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
    ...
    context "when streaming fails on a chunk" do
      ...
      it "handles streaming errors gracefully" do
        ...
        expect(results.last.first.record).to be_a(Outhad::Integrations::Protocol::RecordMessage)
