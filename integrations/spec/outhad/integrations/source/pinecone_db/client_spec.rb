...
RSpec.describe Outhad::Integrations::Source::PineconeDB::Client do
  include WebMock::API
  ...
  describe "#search" do
    context "when the search operation is successful" do
      it "increments the success count" do
        sync_config = Outhad::Integrations::Protocol::VectorConfig.from_json(sync_config_json.to_json)
        response_body = {
          "matches" => [
            {
              "id" => "400",
    ...
    context "when the search operation fails" do
      it "increments the failure count" do
        s_config = Outhad::Integrations::Protocol::VectorConfig.from_json(sync_config_json.to_json)
