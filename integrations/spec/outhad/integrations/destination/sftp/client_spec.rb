...
RSpec.describe Outhad::Integrations::Destination::Sftp::Client do
  include WebMock::API
  ...
  describe "#write" do
    ...
    it "handles write failure with_sftp_client" do
      ...
      expect(response.log).to be_a(Outhad::Integrations::Protocol::LogMessage)
  ...
  describe "#meta_data" do
    it "serves it github image url as icon" do
      image_url = "https://raw.githubusercontent.com/Outhad/outhad/main/integrations/lib/outhad/integrations/destination/sftp/icon.svg"
  ...
  describe "#generate_file_path" do
    it "generate csv file" do
      ...
      expect(file_path).to match(%r{/outhad/test_\d{8}-\d{6}\.csv\z})
    end

    it "generate zip file" do
      file_path = client.send(:generate_file_path, sync_config_compressed_zip)
      expect(file_path).to match(%r{/outhad/test_\d{8}-\d{6}\.zip\z})
    end
  end
end
