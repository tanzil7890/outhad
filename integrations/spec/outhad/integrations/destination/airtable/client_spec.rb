...
RSpec.describe Outhad::Integrations::Destination::Airtable::Client do
  include WebMock::API
  ...
  describe "#check_connection" do
    ...
    it "returns a successful connection status if the request is successful" do
      allow(Outhad::Integrations::Core::HttpClient).to receive(:request).and_return(success_response)
      ...
      expect(message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
      result = message.connection_status
      expect(result.status).to eq(Outhad::Integrations::Protocol::ConnectionStatusType["succeeded"])
    end
    ...
    it "raises an error if the base idis not found in the response" do
      ...
      expect(message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
      result = message.connection_status
      expect(result.status).to eq(Outhad::Integrations::Protocol::ConnectionStatusType["failed"])
      expect(result.message).to eq("base_id not found")
    end

    it "returns a failed connection status if the request is not successful" do
      allow(Outhad::Integrations::Core::HttpClient).to receive(:request).and_return(failure_response)
      message = client.check_connection(connection_config)
      expect(message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
      result = message.connection_status
      expect(result.status).to eq(Outhad::Integrations::Protocol::ConnectionStatusType["failed"])
  ...
  describe "#discover" do
    ...
    before do
      stub_request(:get, Outhad::Integrations::Core::Constants::AIRTABLE_BASES_ENDPOINT)
        .with(headers: { "Authorization" => "Bearer #{connection_config[:api_key]}" })
        .to_return(status: 200, body: base_response, headers: {})

      stub_request(:get, Outhad::Integrations::Core::Constants::AIRTABLE_GET_BASE_SCHEMA_ENDPOINT
      .gsub("{baseId}", connection_config[:base_id]))
        .with(headers: { "Authorization" => "Bearer #{connection_config[:api_key]}" })
        .to_return(status: 200, body: schema_response, headers: {})

      allow(Outhad::Integrations::Destination::Airtable::SchemaHelper)
  ...
  describe "#write" do
    ...
    it "increments the success count" do
      allow(Outhad::Integrations::Core::HttpClient).to receive(:request).and_return(success_response)
      sync_config = Outhad::Integrations::Protocol::SyncConfig.from_json(sync_config_json.to_json)
