...
RSpec.describe Outhad::Integrations::Destination::AmazonS3::Client do
  ...
  describe "#check_connection" do
    it "successfully checks connection" do
      allow_any_instance_of(Outhad::Integrations::Destination::AmazonS3::Client).to receive(:create_connection).and_return(s3_client)
    ...
    it "handles connection failure" do
      allow_any_instance_of(Outhad::Integrations::Destination::AmazonS3::Client).to receive(:create_connection).and_raise(StandardError.new("connection failed"))
  ...
  describe "#write" do
    context "when the write operation is successful" do
      it "increments the success count" do
        s_config = Outhad::Integrations::Protocol::SyncConfig.from_json(sync_config_json.to_json)
        allow_any_instance_of(Outhad::Integrations::Destination::AmazonS3::Client).to receive(:create_connection).and_return(s3_client)
