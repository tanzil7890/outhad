...
RSpec.describe Outhad::Integrations::Destination::Klaviyo::Client do
  include WebMock::API
  ...
  let(:klaviyo_endpoint) { Outhad::Integrations::Destination::Klaviyo::Client::KLAVIYO_AUTH_ENDPOINT }
  let(:klaviyo_payload) { Outhad::Integrations::Destination::Klaviyo::Client::KLAVIYO_AUTH_PAYLOAD }
  let(:headers) do
    {
  ...
  describe "#check_connection" do
    before do
      allow(Outhad::Integrations::Core::HttpClient).to receive(:request)
    end
    ...
    context "when the connection is successful" do
      ...
      before do
        ...
        allow(Outhad::Integrations::Core::HttpClient).to receive(:request).with(klaviyo_endpoint, "POST", payload: klaviyo_payload, headers: headers).and_return(response)
      end
  ...
  describe "#write" do
    ...
    context "when the write is successful" do
      ...
      before do
        allow(Outhad::Integrations::Core::HttpClient).to receive(:request).and_call_original
      end
      ...
      it "sends the correct payload to klaviyo" do
        ...
        expect(Outhad::Integrations::Core::HttpClient).to have_received(:request).with(
