...
RSpec.describe Outhad::Integrations::Destination::MariaDB::Client do
  include WebMock::API
  ...
  describe "#check_connection" do
    context "when the connection is successful" do
      it "returns a succeeded connection status" do
        allow_any_instance_of(Outhad::Integrations::Destination::MariaDB::Client).to receive(:create_connection).and_return(sequel_client)
    ...
    context "when the connection fails" do
      it "returns a failed connection status with an error message" do
        allow_any_instance_of(Outhad::Integrations::Destination::MariaDB::Client).to receive(:create_connection).and_raise(StandardError, "Connection failed")
        message = client.check_connection(sync_config_json[:destination][:connection_specification])
        result = message.connection_status
  ...
  describe "#write" do
    context "when the write operation is successful" do
      before do
        allow_any_instance_of(Outhad::Integrations::Source::MariaDB::Client).to receive(:create_connection).and_return(sequel_client)
      end
