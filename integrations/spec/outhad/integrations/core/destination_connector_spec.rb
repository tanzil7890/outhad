  ...
  module Integrations::Core
    RSpec.describe DestinationConnector do
      ...
      describe "#tracking_message" do
        ...
        it "returns a OuthadMessage with tracking information" do
          connector = described_class.new
          success = 2
          failure = 1

          outhad_message = connector.tracking_message(success, failure, [log_message_data])

          expect(outhad_message).to be_a(Outhad::Integrations::Protocol::OuthadMessage)
          expect(outhad_message.type).to eq("tracking")

          tracking_message = outhad_message.tracking
          expect(tracking_message).to be_a(Outhad::Integrations::Protocol::TrackingMessage)
          ...
          expect(logs.first).to be_a(Outhad::Integrations::Protocol::LogMessage)
